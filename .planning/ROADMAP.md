...
| 24 | Fix Liveblocks Migration Error | Resolve duplicate column `content_draft` error in migration 048. | REQ-1 | `npx wrangler d1 migrations apply ares-db --local` passes without errors. |
| 25 | Database Simplification & Audit | Simplify and audit the 49 migrations and existing schemas. | REQ-2 | An audit log is produced and any coalescing/dropping of obsolete database elements is successfully executed. |
| 26 | Refactor Raw SQL to Kysely | Replace `c.env.DB.prepare` calls with type-safe Kysely syntax in media and auth routes. | REQ-3 | No raw SQL preparation statements exist for queries that can be executed via Kysely; types generate cleanly. |
| 27 | Move Rate Limiting to KV | Refactor isolate-memory global rate limits to a KV-backed implementation. | REQ-4 | Global rate limiting correctly blocks requests across multiple isolates. |
| 28 | Verify Shipped Features & Traceability | Validate Edge Caching, Soft-Delete GC, CI Optimization, and E2E Sharding. | REQ-5, REQ-6, REQ-7, REQ-8 | Verifications run successfully and traceability is updated. |
