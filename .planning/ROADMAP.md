# Roadmap: ARESWEB

## Milestones

- 🚧 **v3.3 Calendar UX Refinement** — Phase 04 (in progress)
- ✅ **v3.2 Operations & UX Refinement** — Phases 01-03 (shipped 2026-04-28)

## Phases

### 🚧 v3.3 Calendar UX Refinement (In Progress)

- [ ] Phase 04: Calendar Hover CSS Fixes (1 plan)

<details>
<summary>✅ v3.2 Operations & UX Refinement (Phases 01-03) — SHIPPED 2026-04-28</summary>

- [x] Phase 01: Calendar Subscriptions (1 plan) — completed 2026-04-28
- [x] Phase 02: Finance Manager Fixes (1 plan) — completed 2026-04-28
- [x] Phase 03: Retroactive Verification Generation (0 plans) — completed 2026-04-28

</details>

## Progress

| Phase             | Milestone | Plans Complete | Status      | Completed  |
| ----------------- | --------- | -------------- | ----------- | ---------- |
| 01. Calendar Subscriptions | v3.2      | 1/1            | Complete    | 2026-04-28 |
| 02. Finance Fixes          | v3.2      | 1/1            | Complete    | 2026-04-28 |
| 03. Verification           | v3.2      | 0/0            | Complete    | 2026-04-28 |
| 04. Hover CSS Fixes        | v3.3      | 0/1            | Not started | -          |

---

### Phase Details

**Phase 04: Calendar Hover CSS Fixes**
Goal: Prevent scrollbars from appearing on event hover.
Requirements: CAL-02
Success criteria:
1. Hovering over a calendar event does not trigger a scrollbar on the parent cell.
2. The calendar layout remains stable and does not shift.
