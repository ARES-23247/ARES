...
- **ts-rest Contract Route Ordering (CRITICAL)**: `createHonoEndpoints` registers Hono routes in the **exact order** they appear in the contract object. Dynamic wildcard segments like `/:slug` or `/:id` will shadow any static routes defined after them (e.g., `/admin`, `/search`). **ALL static and prefixed routes MUST be defined BEFORE any bare dynamic routes in every contract.** Correct order: `GET /` → `GET /search` → `GET /admin` → `GET /admin/:slug/detail` → `GET /:slug` (wildcard LAST). Violating this causes the wildcard handler to intercept requests meant for static routes, returning incorrect responses (typically 404) with no server-side error — making it extremely difficult to diagnose.
