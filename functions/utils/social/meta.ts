...
export async function dispatchFacebook(payload: PostPayload, config: SocialConfig) {
  ...
  return fetch(fbUrl, { signal: AbortSignal.timeout(5000),
    method: "POST",
  ...
  if (config.INSTAGRAM_ACCOUNT_ID && config.INSTAGRAM_ACCESS_TOKEN) {
    promises.push(
      (async () => {
        ...
        const createRes = await fetch(creationUrl, { signal: AbortSignal.timeout(5000), method: "POST", body: creationPayload.toString(), headers: { "Content-Type": "application/x-www-form-urlencoded" } });
        if (!createRes.ok) {
        ...
        if (createData.id) {
          ...
          const pubRes = await fetch(publishUrl, { signal: AbortSignal.timeout(5000), method: "POST", body: publishPayload.toString(), headers: { "Content-Type": "application/x-www-form-urlencoded" } });
          if (!pubRes.ok) {
  ...
  if (config.FACEBOOK_PAGE_ID && config.FACEBOOK_ACCESS_TOKEN) {
    promises.push(
      (async () => {
        ...
        const res = await fetch(fbUrl, { signal: AbortSignal.timeout(5000), method: "POST", body: fbPayload.toString(), headers: { "Content-Type": "application/x-www-form-urlencoded" } });
        if (!res.ok) {
