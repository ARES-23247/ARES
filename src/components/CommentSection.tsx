...
export default function CommentSection({ targetType, targetId, isAdmin }: CommentSectionProps) {
  ...
  const fetchComments = () => {
    fetch(`/api/comments/${targetType}/${targetId}`, { credentials: "include" })
      .then(r => r.json())
      .then((data) => {
        const typed = data as { comments: Comment[]; authenticated: boolean };
      ...
      {isAuthenticated ? (
        <div className="flex gap-3 mb-8">
          ...
          <button onClick={submitComment} disabled={posting || !newComment.trim()}
            className="px-4 bg-ares-red hover:bg-red-700 text-white rounded-xl font-bold text-sm disabled:opacity-50 transition-colors flex items-center gap-1.5 self-end"
          >
