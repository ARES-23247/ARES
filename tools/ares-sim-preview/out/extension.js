...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerCodeLensProvider({ scheme: 'file', language: 'typescriptreact' }, simLensProvider));
    // Command: Preview Sim
    let previewPanel = undefined;
    context.subscriptions.push(vscode.commands.registerCommand('ares.previewSim', async (document) => {
        ...
        if (previewPanel) {
            ...
            previewPanel = vscode.window.createWebviewPanel('aresSimPreview', `Preview: ${simId}`, vscode.ViewColumn.Beside, {
                enableScripts: true,
    ...
    // Command: Debug Sim
    context.subscriptions.push(vscode.commands.registerCommand('ares.debugSim', async (document) => {
        const simId = getSimIdFromFile(document.uri);
        if (!simId)
            return;
        ensureViteRunning();
